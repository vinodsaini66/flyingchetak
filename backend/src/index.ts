...
const xValueGet = async () => {
  ...
  XInterval = setInterval(async () => {
    ...
    currentGame = await Game.findOne({_id:ongoingGame?.current_game,is_game_end:false});

    if (!currentGame) {
      // clearInterval(XInterval); 
      await GameController.newGame(52)
     // return; // No current game, stop fetching data
    }
    const xData: {
      message: string;
    ...
    if (currentGame?.end_time < Date.now() && !gameInterval) {
      ...
      gameDataGet();
      setTimeout(async () => {
        (gameInterval = false), await xValueGet();
      }, 10000);
    }
    // else{
